...
type ProjectGroup = 'Main' | 'Mini' | 'Spark';
type Project = {
...
const heroHighlights = [
  { icon: Code2, title: 'Problem Solving', text: 'Strong interest in logical thinking and DSA.' },
  { icon: Terminal, title: 'Full Stack Dev', text: 'Frontend, backend and database-driven systems.' },
  { icon: Zap, title: 'API Integration', text: 'Hands-on experience with REST APIs and services.' },
];
      ...
      <main>
        <section id="home" className="hero section-pad">
          <div className="hero-copy reveal">
            ...
            <h1>Software Developer crafting scalable, user-focused applications with Java and modern web technologies.</em> problems.</h1>
            ...
            <div className="hero-mini-stats">{heroStats.map(([value, label]) => <div className="mini-stat" key={label}><strong>{value}</strong><span>{label}</span></div>)}</div>
            <div className="hero-socials">{socials.map(({ label, href, icon: Icon }) => <a key={label} href={href} target="_blank" rel="noreferrer"><Icon size={16} /> {label}</a>)}</div>
          </div>
          ...
          <div className="hero-highlights reveal reveal-delay">
