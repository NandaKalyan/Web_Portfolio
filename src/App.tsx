...
function App() {
  ...
  const scrollTo = (id: string) => {
    ...
    // Keep the existing behavior unchanged on smaller screens.
    if (window.innerWidth <= 900) {
      element.scrollIntoView({ behavior: 'smooth' });
      return;
    }

    // Adjust section positioning only on laptop/desktop screens.
    const navHeight = 78;
    const sectionPaddingTop = 140;
    const targetPosition =
      element.getBoundingClientRect().top +
      window.scrollY -
      (sectionPaddingTop - navHeight);
  ...
  return (
    <div className="site-shell">
      <div className={`page-loader ${loading ? 'is-visible' : ''}`}><span>NK</span><div className="loader-line"><i /></div></div>
      <div className="cursor-dot" style={{ left: cursor.x, top: cursor.y }}><span>{cursor.label}</span></div>
      ...
      <header className={`site-header ${scrolled ? 'scrolled' : ''}`}>
        ...
        <nav className={menuOpen ? 'open' : ''}>
          {['home', 'about', 'skills', 'journey', 'projects', 'contact'].map((item) => (
            <a key={item} href={`#${item}`} onClick={(event) => { event.preventDefault(); scrollTo(item); }}>{item}</a>
          ))}
        </nav>
        <button className="connect-button" onClick={() => scrollTo('contact')}>Let's connect <ArrowUpRight size={16} /></button>
        <button className="menu-button" onClick={() => setMenuOpen((open) => !open)} aria-label="Toggle menu">{menuOpen ? <X size={22} /> : <Menu size={22} />}</button>
      </header>
      ...
      <main>
        <section id="home" className="hero">
          <div className="hero-grid">
            <div className="hero-copy">
              <span className="eyebrow">SOFTWARE DEVELOPER / JAVA DEVELOPER</span>
              <h1>Building useful<br /><em>things with code.</em></h1>
              <p>I build practical software with Java, modern web technologies and a problem-solving mindset — from backend systems to responsive interfaces.</p>
              <div className="hero-actions">
                <button className="button button-primary" onClick={() => scrollTo('projects')}>View selected work <ArrowDownRight size={17} /></button>
                <button className="button button-quiet" onClick={() => scrollTo('contact')}>Let's talk <ArrowUpRight size={17} /></button>
              </div>
            </div>
            <div className="hero-visual">
              <div className="visual-frame"><img src="/assets/images/Professional_Profile.png" alt="Nanda Kalyan professional profile" /><span className="visual-label">01 / PROFILE</span><span className="visual-corner tl" /><span className="visual-corner br" /></div>
              <div className="hero-note">Based in India<br />Open to opportunities</div>
            </div>
          </div>
        ...
        <section id="about" className="section-pad about-section">
          <div className="section-heading"><span className="section-number">01 / ABOUT</span><h2>Curious by nature.<br /><em>Builder by choice.</em></h2></div>
          <div className="about-grid">
            <div className="about-copy">
              <p>I'm a Java Full Stack Developer with a foundation in software engineering and hands-on experience building practical applications across web, mobile, AI-assisted tools, APIs and database-driven systems.</p>
              <p>I enjoy transforming ideas into functional software through problem solving, clean development practices and continuous learning.</p>
              <button className="text-link" onClick={() => scrollTo('journey')}>Explore my journey <ChevronRight size={16} /></button>
            </div>
            <div className="strength-grid">
              {[['Problem solving', '01', 'Strong interest in logical thinking and DSA.'], ['Full stack development', '02', 'Experience working across frontend, backend and databases.'], ['Java development', '03', 'Strong foundation in Java and Java-based web development.'], ['API integration', '04', 'Hands-on experience consuming REST APIs and external services.']].map(([title, number, text]) => (
                <div className="strength" key={number}><span>{number}</span><h3>{title}</h3><p>{text}</p><ArrowUpRight size={17} /></div>
              ))}
            </div>
          </div>
        </section>
        ...
        <section id="skills" className="section-pad skills-section">
          <div className="section-heading split-heading">
            <div><span className="section-number">02 / TOOLKIT</span><h2>Tools for turning<br /><em>thought into form.</em></h2></div>
            <p>Not a list of percentages. Just the technologies I use to think, build, test and ship useful software.</p>
          </div>
        ...
        <section id="journey" className="section-pad journey-section">
          <div className="section-heading"><span className="section-number">03 / BACKGROUND</span><h2>The path so far.</h2></div>
          <div className="timeline">
            <div className="timeline-item">
              <div className="timeline-meta">2024 — 2025<br /><span>TRAINING</span></div>
              <div className="timeline-line"><span /></div>
              <div className="timeline-content"><h3>Tap Academy</h3><p>Java Full Stack Development</p><small>Java · Advanced Java · SQL · React · Spring · Hibernate · Full Stack Development</small></div>
            </div>
            <div className="timeline-item">
              <div className="timeline-meta">2023<br /><span>INTERNSHIP</span></div>
              <div className="timeline-line"><span /></div>
              <div className="timeline-content"><h3>Prinston Smart Engineers</h3><p>Full Stack Development Intern</p></div>
            </div>
            <div className="timeline-item">
              <div className="timeline-meta">2020 — 2024<br /><span>EDUCATION</span></div>
              <div className="timeline-line"><span /></div>
              <div className="timeline-content"><h3>Global Academy of Technology</h3><p>Bachelor of Engineering · Information Science and Engineering</p><small>CGPA: 8.94</small></div>
            </div>
          </div>
          <div className="dsa-panel">
            <div><span className="section-number">A PRACTICE IN PROGRESS</span><strong>150<span>+</span></strong><p>DSA problems solved</p></div>
            <div className="dsa-copy"><Terminal size={21} /><p>Practiced Data Structures and Algorithms with a focus on logical problem solving and Java-based implementation.</p></div>
          </div>
        </section>
        ...
        <section id="projects" className="section-pad projects-section">
          ...
          <div className="project-grid">
            {visibleProjects.map((project, index) => (
              <article className="project-card" key={project.title} onClick={() => setSelectedProject(project)} onMouseEnter={() => setCursor((current) => ({ ...current, label: 'VIEW' }))} onMouseLeave={() => setCursor((current) => ({ ...current, label: '' }))}>
                <div className={`project-art tint-${project.tint}`}><img src={project.image} alt={`${project.title} screenshot`} loading="lazy" /><span className="art-index">0{index + 1}</span><span className="art-overlay" /></div>
                <div className="project-info">
                  <div className="project-topline"><span>{project.category}</span><span>{String(index + 1).padStart(2, '0')} / 11</span></div>
                  <h3>{project.title}</h3>
        ...
        <section id="contact" className="section-pad contact-section">
          <div className="contact-frame">
            <div>
              ...
              <p>I'm always interested in learning, building and taking on new software development challenges.</p>
              <div className="contact-links">
                <a href="mailto:nandakalyan2002@gmail.com"><Mail size={18} /><span>Email me<small>nandakalyan2002@gmail.com</small></span><ArrowUpRight size={16} /></a>
                {socials.map(({ label, href, icon: Icon }) => <a href={href} target="_blank" rel="noreferrer" key={label}><Icon size={18} /><span>{label}<small>Open profile</small></span><ArrowUpRight size={16} /></a>)}
              </div>
            </div>
            <form onSubmit={handleSubmit}>
              <label>Name<input value={formState.name} onChange={(e) => setFormState((s) => ({ ...s, name: e.target.value }))} placeholder="Your name" required /></label>
              <label>Email<input type="email" value={formState.email} onChange={(e) => setFormState((s) => ({ ...s, email: e.target.value }))} placeholder="you@example.com" required /></label>
              <label>Subject<input value={formState.subject} onChange={(e) => setFormState((s) => ({ ...s, subject: e.target.value }))} placeholder="How can we work together?" /></label>
              <label>Message<textarea rows={4} value={formState.message} onChange={(e) => setFormState((s) => ({ ...s, message: e.target.value }))} placeholder="Tell me a little about your idea..." required /></label>
              <button className="button button-primary" type="submit" disabled={formStatus === 'sending'}>{formStatus === 'sending' ? 'Sending...' : formStatus === 'success' ? 'Message sent!' : formStatus === 'error' ? 'Try again' : 'Send message'} {formStatus === 'success' ? <Check size={16} /> : <Send size={16} />}</button>
              {formStatus === 'error' && <p className="form-error">Something went wrong. Please try again or email me directly.</p>}
              {formStatus === 'success' && <p className="form-success">Thanks! I'll get back to you shortly.</p>}
            </form>
          </div>
        </section>

        <section className="resume-cta section-pad">
          <div><span className="section-number">OPEN TO WHAT'S NEXT</span><h2>Ready to build<br /><em>something great?</em></h2></div>
          <div>
            <p>Explore my projects, view my work and connect with me for software development opportunities.</p>
            <div className="hero-actions">
              <a className="button button-primary" href="/NandaKalyan_ATS.pdf" download="Nanda-Kalyan-Resume.pdf">Download resume <ArrowUpRight size={17} /></a>
              <button className="button button-quiet" onClick={() => scrollTo('projects')}>View projects <ArrowDownRight size={17} /></button>
            </div>
          </div>
        </section>
      </main>
      ...
      <footer>
        <div className="footer-main">
          <a className="brand" href="#home"><span>NK</span><small>SOFTWARE DEVELOPER</small></a>
          <p>Software Developer · Java Developer · Front End Developer</p>
          <div className="footer-socials">{socials.map(({ label, href, icon: Icon }) => <a key={label} href={href} target="_blank" rel="noreferrer"><Icon size={17} /> {label}</a>)}</div>
        </div>
        <div className="footer-bottom"><span>© 2026 Nanda Kalyan. All rights reserved.</span><span>Built with intention <Sparkles size={13} /></span></div>
      </footer>

      {selectedProject && (
        <div className="modal-backdrop" onClick={() => setSelectedProject(null)}>
          <div className="project-modal" onClick={(event) => event.stopPropagation()}>
            <button className="modal-close" onClick={() => setSelectedProject(null)} aria-label="Close project details"><X size={20} /></button>
            <div className={`modal-art project-art tint-${selectedProject.tint}`}><img src={selectedProject.image} alt={`${selectedProject.title} screenshot`} /></div>
            <div className="modal-copy">
              <div className="project-topline"><span>{selectedProject.category}</span><span>PROJECT DETAIL</span></div>
              <h2>{selectedProject.title}</h2>
              <p>{selectedProject.description}</p>
              <div className="modal-columns">
                <div><h4>FEATURES</h4>{selectedProject.features.map((item) => <span className="check-item" key={item}><Check size={14} />{item}</span>)}</div>
                <div><h4>TECHNICAL HIGHLIGHTS</h4>{selectedProject.highlights.map((item) => <span className="check-item" key={item}><Zap size={14} />{item}</span>)}</div>
              </div>
              <div className="project-links modal-links">
                <a href={selectedProject.githubUrl} target="_blank" rel="noreferrer"><Github size={15} /> GitHub <ArrowUpRight size={14} /></a>
                {selectedProject.liveDemoUrl && <a href={selectedProject.liveDemoUrl} target="_blank" rel="noreferrer"><ExternalLink size={15} /> Live Demo <ArrowUpRight size={14} /></a>}
                {selectedProject.demoVideoUrl && <a href={selectedProject.demoVideoUrl} target="_blank" rel="noreferrer"><Play size={14} /> Demo Video <ArrowUpRight size={14} /></a>}
              </div>
            </div>
          </div>
        </div>
